...
int read_encrypt_vec_from_file_comp_inter_sec_prod( int vsizelocal, const char * input_encr_file_name, const char * input_tfidf_vec_file_name, const char * input_bin_vec_file_name, const char * output_file_name, const char * key_file_name);
...
int read_encrypt_vec_from_file_comp_inter_sec_prod( int vsizelocal, const char * input_encr_file_name, const char * input_tfidf_vec_file_name, const char * input_bin_vec_file_name, const char * output_file_name, const char * key_file_name)
{
  ...
  mpz_t co_ord_factor;
  mpz_t random_no;
	FILE *input_encr_file, *input_tfidf_vec_file, *input_bin_vec_file, *output_file;
	
	
	vsize = vsizelocal;
	input_encr_file = fopen(input_encr_file_name, "r");
  ...
  mpz_init(random_no);
	...
	if (input_encr_file == NULL) {
		printf("\n%s", "Error: open input_encr_file!");
		return -2;
	}
  ...
  if (output_file == NULL) {
	...
	while ( (input_size < vsizelocal) )
	{
		...
		{
			gmp_fscanf(input_encr_file,"%Zd", (vec1+input_size));
		}
		else
		{
			gmp_fscanf(input_encr_file,"%Zd\n", (vec1+input_size));
		}
	...
	{
		fprintf(stderr, "%s:%d::ERROR! Nothing to read OR parsing error! input_size:%d, vsizelocal:%d\n", 
  ...
  //compute encrypted the vec1 * co_ord_factor (dot product)
  for (i = 0; i < input_size; i++) {
    //compute m1 * m2
    mpz_powm_ui(big_temp, *(vec1+i), *(p_bin_vec+i), n_square);
    //compute m1 + m2
  ...
  mpz_out_str(output_file, 10, random_no);
    fprintf(output_file, "\n");
  //Calculate randomized cosine tfidf product
  mpz_add(cosine_result, cosine_result, random_no);
  //Compute modulus
  mpz_mod(cosine_result, cosine_result, n_square);
  ...
  if (mpz_out_str(output_file, 10, cosine_result) == 0)
    ...
    fprintf(output_file, "\n");
	#endif
	gmp_printf("\nThus co-ord. factor of %s and %s score = %Zd written in %s\n", input_encr_file_name, input_bin_vec_file_name, co_ord_factor, output_file_name);
	#if 0
...
*/
  ...
  int main(int argc, char *argv[])
  {
	...
	if (argc != 6){
		printf("Usage: %s num_vec_dim input_encr_file tfidf_vect_input_file2 bin_vect_input_file3 output_file_prod\n", argv[0]);
		exit(1);
	}

	//Dynamically creating the array

	return read_encrypt_vec_from_file_comp_inter_sec_prod(atoi(argv[1]), argv[2], argv[3], argv[4], argv[5], KEY_FILE_NAME);
